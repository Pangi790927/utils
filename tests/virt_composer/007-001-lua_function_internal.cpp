...
/* c_function_t wraps a native C++ callback and exposes it as a callable Lua value through the
"__vc_metatable" __call metamethod (see luaopen_vc()'s __call handler, which dispatches to
c_function_t::call()). Callbacks are registered ahead of time with
c_function_t::add_internal_func(name, fn) - this is exactly the pattern
tests/test_vulkan_composer.cpp uses for "fill_buffer_with_quad_vertices" - then a yaml node with
`m_type: vc::c_function_t` and `m_source: "[INTERNAL]"` looks the name up in that registry
(c_function_t::init()) and binds it. Once bound, a Lua script can call it as `vc.<name>(...)`. */

static int g_last_arg_seen = -1;

int test7_internal_func_called_from_lua() {
    /* c_function_t::init() looks the registry up by m_name, which is set to the YAML object's
    own key ("doubler" below) - not by any name chosen here - so the registered name must match
    that key exactly. */
    vc::c_function_t::add_internal_func("doubler",
        [](lua_State *L) -> int {
    ...
    auto fn_obj = vc::get_ref<vc::c_function_t>(vs.get(), "doubler");
...
int test7_unknown_internal_func_fails_init() {
    /* c_function_t::init() returns VC_ERROR_GENERIC when m_source == "[INTERNAL]" but no
    matching name was ever registered via add_internal_func() - c_function_t::create() turns
    that into a thrown vc::except_t ("Failed c_function_t init"), which build_object() lets
