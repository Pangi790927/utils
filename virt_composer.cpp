...
std::shared_ptr<virt_state_t> create_state() {
    ...
    /* Registered directly (not via VC_REGISTER_MEMBER_FUNCTION) so push() gets the real
    calling L rather than a converted argument - see lua_object_t's doc comment. */
...
void lua_object_t::capture_ref(lua_State *L) {
    ...
    /* An empty stack isn't nil on top: lua_isnil() on an empty stack reads LUA_TNONE, not
    LUA_TNIL, and returns false - so this needs its own check. */
    if (lua_gettop(L) < 1)
        return;

    /* Capturing nil is a release, not a "captured nil" - luaL_ref on nil returns LUA_REFNIL, which
    push()/call()'s ref == LUA_NOREF guard wouldn't recognize as empty. */
    if (lua_isnil(L, -1)) {
        lua_pop(L, 1);
        return;
    }
...
Core:
  - On return Lua holds no strong reference to `obj`; if it held the last one the object is
    already destroyed, not merely eligible. Lua's collector is tracing, so an unreferenced object
    can outlive forced collectgarbage("collect") passes (2026-09-04: one died only at teardown).
  - `obj` is unregistered from the name table and no wrapper for it survives anywhere, so it can
    no longer be found by name and a later object can never be handed this wrapper.
  - Every Lua handle to `obj` shares that one wrapper and is left alive but empty; none of them
    may be used again, though __gc still runs on them.
